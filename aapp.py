...
st.markdown("Google OR-Tools CP-SAT Solver tabanlı gelişmiş optimizasyon arayüzü.")
...
# =========================================================
# DİNAMİK PARAMETRELER (SEKME/EXPANDER YAPISI)
# =========================================================
with st.sidebar:
    st.header("⚙️ Ayarlar")
    with st.expander("🏗️ Hat Parametreleri", expanded=True):
        L = st.number_input("Maksimum Yürüme Mesafesi (L)", value=4)
        D = st.number_input("Hedef Üretim Miktarı (D)", value=32)
        T = st.number_input("Vardiya Süresi (T - dk)", value=510)
    
    with st.expander("⚖️ Optimizasyon Kısıtları", expanded=False):
        U_MAX = st.slider("Maks. Operatör Doluluğu (U_MAX)", 0.1, 1.0, 1.0)
...
def solve_model(exact_workers=None):
    ...
    for w in W:
        model.Add(D * sum(q[w, j] for j in J) <= int(round(U_MAX * T * SCALE)))
    for w in W:
        for j in J:
            for k in J:
    ...
    solver.parameters.max_time_in_seconds = 20 # Hızlı sonuç için 20sn yeterli
    ...
    solution = {
        ...
        "opened_stations": sum(solver.Value(s[j]) for j in J),
        ...
        "meets_target": (T / C_value >= D - 1e-6) if C_value > 0 else False,
    }
    for i in I:
        for j in J:
...
if st.button("🚀 Tüm Senaryoları Hesapla ve Analiz Et"):
    with st.spinner("1'den 36 Operatöre kadar tüm senaryolar hesaplanıyor..."):
        all_results = {}
        # Tüm operatör sayıları için döngü (1-36)
        for eps in range(1, 37):
            all_results[eps] = solve_model(exact_workers=eps)
        ...
        for eps in range(1, 37):
            res = all_results[eps]
            if res is None:
                summary_list.append([eps, "Çözüm Yok", "-", "-", "Hayır"])
            else:
                summary_list.append([
                    eps, 
                    f"{res['C']:.2f} dk", 
                    f"{res['opened_stations']} Adet",
                    f"{res['reachable_output']:.2f} Adet", 
                    "✅" if res['meets_target'] else "❌"
                ])
        
        df_all = pd.DataFrame(summary_list, columns=["Operatör Sayısı", "Min. Çevrim Süresi (C)", "Açık İstasyon", "Ulaşılabilir Üretim", "Hedef Karşılandı mı?"])
        
        st.subheader("📊 Tüm Operatör Senaryoları (1 - 36)")
        st.dataframe(df_all, use_container_width=True, hide_index=True)

        # Detaylı Rapor (Slider'da seçilen operatör için)
        if target_workers in all_results and all_results[target_workers] is not None:
            res = all_results[target_workers]
            st.markdown("---")
            st.success(f"🎯 Seçilen Operatör Sayısı ({target_workers}) İçin Detaylar")
            
            m1, m2, m3 = st.columns(3)
            m1.metric("Çevrim Süresi", f"{res['C']:.2f} dk")
            m2.metric("Üretim Kapasitesi", f"{res['reachable_output']:.2f} Adet")
            m3.metric("Hedef Uyumu", "Evet" if res['meets_target'] else "Hayır")

            t1, t2 = st.tabs(["İstasyon Bazlı Dağılım", "Operatör Verimliliği"])
            with t1:
                s_data = [[j, str(res['ops_of_station'][j]), f"{res['station_loads'][j]:.2f} dk"] for j in J if res['station_loads'][j] > 0]
                st.table(pd.DataFrame(s_data, columns=["İstasyon", "Operasyonlar", "Yük"]))
            with t2:
                w_data = [[w, str(res["stations_of_worker"][w]), f"%{res['worker_U'][w]:.2f}"] for w in W if len(res["stations_of_worker"][w]) > 0]
                st.table(pd.DataFrame(w_data, columns=["Operatör", "İstasyonlar", "Doluluk (U)"]))
